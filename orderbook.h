...
public:
	order_level_info get_l2_snapshot(size_t depth)  const {
		order_level_infos bids;
		order_level_infos asks;

		{
			...
			for (const auto& [p, lst] : bids_) {
				...
				for (const auto& ord : lst) {
					total_qty += ord->get_remaining_quantity();
				}
				bids.push_back({ p, total_qty });
			...
			for (const auto& [p, lst] : asks_) {
				...
				asks.push_back({ p, total_qty });
			}

		}
		return order_level_info(bids, asks);
	}
	...
	trades add_order(order_ptr order)
	{
		...
		//logic for good till cancel
		order_type t = order->get_order_type();
		order_ptrs* list_ptr{};
		if (s == side::buy)		list_ptr = &bids_[p]; // inserting into the bids_ map
		else					list_ptr = &asks_[p]; // inserting into the asks_ map
		if (order->get_order_type() == order_type::good_till_cancel) {
			list_ptr->push_back(order);
			auto it = std::prev(list_ptr->end());

			orders_[order->get_order_id()] = { order, it };

			if (can_match(s, p)) return match_orders();
		}
		//fill or kill logic
